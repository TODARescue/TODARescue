...
$getMembersQuery = "
    ...
    WHERE cm.circleId = $circleId AND cm.isSharing = 0
";
...
$getRoleQuery = "
    ...
    WHERE cm.userId = $userId AND cm.circleId = $circleId AND cm.isSharing = 0
";
...
if ($getRoleResult && mysqli_num_rows($getRoleResult) > 0) {
    while ($row = mysqli_fetch_assoc($getRoleResult)) {
        $role = $row['role'];
    }
}
